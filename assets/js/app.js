...
let statsChartStati = null;
let statsChartMacchinari = null;
...
window.loadStats = async () => {
    ...
    const snapshotManutenzioni = await getDocs(qManutenzioni);
    const manutenzioni = [];
    snapshotManutenzioni.forEach(doc => manutenzioni.push(doc.data()));

    const qComponenti = query(collection(db, "componenti"), where("userId", "==", currentUser.uid));
    const snapshotComponenti = await getDocs(qComponenti);
    const componenti = [];
    snapshotComponenti.forEach(doc => componenti.push(doc.data()));
    
    const qMacchinari = query(collection(db, "macchinari"), where("userId", "==", currentUser.uid));
    const snapshotMacchinari = await getDocs(qMacchinari);
    const macchinariMap = {};
    snapshotMacchinari.forEach(doc => macchinariMap[doc.id] = doc.data().nome);
    ...
    const interventiMese = manutenzioni.filter(m => {
        const d = new Date(m.data);
        return d.getMonth() === currentMonth && d.getFullYear() === currentYear;
    }).length;
    
    let componentiUsatiCount = 0;
    manutenzioni.forEach(m => {
        if(m.componentiUsati) {
            m.componentiUsati.forEach(c => componentiUsatiCount += parseInt(c.quantita || 0));
        }
    });
    
    const componentiScarsi = componenti.filter(c => parseInt(c.quantita) <= parseInt(c.scortaMin)).length;
    
    document.getElementById('stat-interventi-mese').textContent = interventiMese;
    document.getElementById('stat-componenti-usati').textContent = componentiUsatiCount;
    document.getElementById('stat-componenti-scarsi').textContent = componentiScarsi;
    document.getElementById('stat-media-tempo').textContent = "-"; 
    ...
    const statiCounts = {}; 
    manutenzioni.forEach(m => {
        let s = m.stato || 'Sconosciuto';
        s = s.charAt(0).toUpperCase() + s.slice(1);
        statiCounts[s] = (statiCounts[s] || 0) + 1;
    });
    
    // 2. Macchinari
    const macchinariCounts = {};
    manutenzioni.forEach(m => {
        const name = macchinariMap[m.macchinarioId] || 'Sconosciuto';
        macchinariCounts[name] = (macchinariCounts[name] || 0) + 1;
    });
    ...
    for(let i=5; i>=0; i--) {
        ...
        const count = manutenzioni.filter(m => {
    ...
    renderChart('chart-stati', 'doughnut', Object.keys(statiCounts), Object.values(statiCounts), ['#1a5490', '#e0e0e0', '#000000', '#888888']);
    renderChart('chart-macchinari', 'bar', Object.keys(macchinariCounts), Object.values(macchinariCounts), '#1a5490');
    renderChart('chart-trend', 'line', trendLabels, trendData, '#1a5490');
};

function renderChart(canvasId, type, labels, data, colors) {
    const ctx = document.getElementById(canvasId).getContext('2d');
    
    if(canvasId === 'chart-stati' && statsChartStati) statsChartStati.destroy();
    if(canvasId === 'chart-macchinari' && statsChartMacchinari) statsChartMacchinari.destroy();
    if(canvasId === 'chart-trend' && statsChartTrend) statsChartTrend.destroy();
    ...
    if(canvasId === 'chart-stati') statsChartStati = chart;
    if(canvasId === 'chart-macchinari') statsChartMacchinari = chart;
    if(canvasId === 'chart-trend') statsChartTrend = chart;
}
